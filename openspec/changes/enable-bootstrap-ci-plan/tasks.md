...
- [x] 2.3 [ユーザー作業] PR #43でのplan実地確認(タスク3.1)で`storage.buckets.get`権限不足が判明したため追加した`roles/storage.legacyBucketReader`(バケットスコープ)を、同じ手動apply手順で反映する(実際に`Plan: 1 to add, 0 to change, 0 to destroy.`で当該ロールのみ追加されたことを確認)

## 3. 動作確認 — [ユーザーが手動で実行する作業]

- [x] 3.1 [ユーザー作業] PR #43(本change自体のPR、`terraform/bootstrap/main.tf`を変更しているため実地確認を兼ねる)で、タスク2.3の権限反映後に`plan-bootstrap` jobを再実行し、CI上で`terraform plan`が正常終了(`No changes`または想定通りの差分)し、PRにplan結果がコメントされることを確認する(全ステップ成功、`No changes.`を確認)
- [x] 3.2 [ユーザー作業] 同じPR上で、`terraform-apply.yml`が起動していない(=applyが自動実行されていない)ことを確認する(`enable-bootstrap-ci-plan`ブランチに対する`terraform-apply.yml`の実行履歴は0件であることを確認)
